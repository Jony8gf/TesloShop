...
const HomePage: NextPage<Props> = ({ session }) => {
  ...
  const [productsCookies, setProductsCookies] = useState<IProduct[]>([]);
  ...
  useEffect(() => {
    ...
    console.log(cookies);
    console.log(userId);
    tesloApi.delete('admin/productscookies', { data: { cookies: Cookies.get('token'), userId: session.user._id || null} });
  ...
  return (
    <ShopLayout title={'TesloShop - Home'} pageDescription={'Encuentra los mejores de Teslo aquí'}>
      ...
      <ProductListCookies products={products} />
